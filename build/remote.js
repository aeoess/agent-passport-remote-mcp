...
const MCP_SERVER_VERSION = '2.21.1';
...
function loadStats() {
    try {
        if (existsSync(STATS_FILE)) {
            ...
            if (!loaded.toolCalls)
                loaded.toolCalls = {};
            if (!loaded.passportsIssued)
                loaded.passportsIssued = 0;
            return loaded;
        }
    }
    catch { }
...
// ═══════════════════════════════════════
async function registerAgentWithGateway(mcpResponse) {
    if (!GATEWAY_API_KEY)
        return;
    try {
...
function spawnMCPProcess(sessionId) {
...
app.get('/health', (_req, res) => {
    res.json({ status: 'ok', server: 'agent-passport-remote-mcp', version: '2.21.1', sessions: sessions.size, maxSessions: MAX_SESSIONS, uptime: process.uptime() });
});
...
app.get('/.well-known/agent.json', (_req, res) => {
    res.json({
        ...
        url: 'https://mcp.aeoess.com', version: '2.21.1',
...
app.get('/sse', authMiddleware, (req, res) => {
    ...
    const session = { id: sessionId, process: child, sseResponse: res, created: Date.now(), lastActivity: Date.now(), pendingPassportIds: new Set() };
...
app.post('/mcp', authMiddleware, async (req, res) => {
    ...
    let responded = false;
    const timeout = setTimeout(() => { if (!responded) {
        responded = true;
        try {
            child.kill();
        }
        ...
        ;
        res.status(504).json({ error: 'MCP timeout' });
    } }, 30000);
    const rl = createInterface({ input: child.stdout });
    rl.on('line', (line) => {
    ...
    child.on('error', (err) => { if (!responded) {
        responded = true;
        clearTimeout(timeout);
        res.status(500).json({ error: err.message });
    } });
    child.on('exit', (code) => { if (!responded) {
        responded = true;
        clearTimeout(timeout);
        res.status(500).json({ error: `Process exited ${code}` });
    } });
    ...
    try {
        ...
        setTimeout(() => { try {
            recordToolName(req.body);
            child.stdin.write(JSON.stringify({ ...req.body, id: (req.body.id || 2) }) + '\n');
        }
        catch { } }, 100);
    }
    catch (err) {
...
<body><h1>Agent Passport System</h1><p><span class="badge">v2.21.1 — 2497 tests — 132 tools</span></p>
